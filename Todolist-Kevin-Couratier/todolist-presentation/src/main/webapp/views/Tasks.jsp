...
<body>
	...
	<ul class="collection">
		<c:forEach items="${tasksNotFinished}" var="task">
			<li class="collection-item">
				${task.label} terminant le 
				<fmt:formatDate type="both" dateStyle="short" timeStyle="short" value="${task.endDate}" /></li>
		</c:forEach>
	</ul>
	
	<a href="<c:url value="/tasks/finish/all"/>" onclick="confirm('Etes-vous sûr de vouloir terminer toutes les tâches ?');">Terminer toutes mes tâches</a>
